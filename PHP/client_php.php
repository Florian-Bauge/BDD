...
function getArrayClient($text){// Récupère les données nécessaire pour l'affichage de la page client
    $array=array();
    $mysqli = Connect();
    $sql ="SELECT `code_client`, `name`, `Email`, `Phone`, `nom`FROM `client` INNER JOIN `grillepoint` ON client.id_membership=grillepoint.id_membership WHERE client.code_client LIKE '".$text."%' OR client.name LIKE '".$text."%';";
